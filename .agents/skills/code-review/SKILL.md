---
name: code-review
description: Review the changes since a fixed point (commit, branch, tag, or merge-base) along three axes — Standards (does the code follow this repo's documented coding standards?), Spec (does the code match what the originating issue/spec asked for?), and Overengineering (does it do more than it needs to?). Runs all three reviews in parallel sub-agents and reports them side by side. Use when the user wants to review a branch, a PR, work-in-progress changes, or asks to "review since X".
---
...
- **Overengineering** — does the code do more than it needs to? (via `/ponytail-review`)
...
The issue tracker should have been provided to you — check `docs/agents/issue-tracker.md`.
...
**Overengineering sub-agent prompt** — include:

- The full diff command and commit list.
- The brief: "Run `/ponytail-review` on this diff. Find what to delete: reinvented standard library, unneeded dependencies, speculative abstractions, dead flexibility. One line per finding: location, what to cut, what replaces it. Under 300 words."
...
- Code that follows every standard and does exactly what was asked, but adds an abstraction nobody needs → **Standards pass, Spec pass, Overengineering fail.**
