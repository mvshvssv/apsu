    ...
    'no-refs-in-subject-body': ({ subject, body }) => {
      for (const [field, text] of [
        ...
        if (text && /#\d+/.test(text)) {
          return [
            false,
            `Issue refs (#N) not allowed in ${field} — use footer only`,
          ];
        }
      }
      return [true, ''];
    ...
    'body-paragraph-format': ({ body }) => {
      ...
      for (const para of body
        ...
        if (!/^[A-Z]/.test(para))
          return [
            false,
            `Body paragraph must start uppercase: "${para.slice(0, 50)}"`,
          ];
        if (!para.endsWith('.'))
          return [
            false,
            `Body paragraph must end with period: "${para.slice(-50)}"`,
          ];
      }
      return [true, ''];
    },
      ...
      for (const line of footer.split('\n')) {
        ...
        while ((m = re.exec(line)) !== null) {
          ...
          const segment = before
            .slice(Math.max(0, before.lastIndexOf(',') + 1))
            .trim();
          if (!/^(?:close[sd]?|fix(?:e[sd])?|resolve[sd]?)$/i.test(segment)) {
            return [
              false,
              `Footer ref ${m[0]} must be preceded by a closing keyword`,
            ];
          }
        }
      }
